...
export default function DatabaseClient() {
  ...
  const [isAdminMutationAnimating, setIsAdminMutationAnimating] = useState(false);
  ...
  async function runWithAdminMutationShimmer(task: () => Promise<void>) {
    ...
    setIsAdminMutationAnimating(true);
    try {
      await task();
    } finally {
      const elapsed = Date.now() - startedAt;
      const remaining = Math.max(0, minAnimationMs - elapsed);
      window.setTimeout(() => setIsAdminMutationAnimating(false), remaining);
    }
  }
  ...
  async function handleSaveChanges() {
    ...
    await runWithAdminMutationShimmer(async () => {
      setIsSaving(true);
      setSaveMessage(null);
      try {
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Box sx={{ px: { xs: 1.5, md: 3 }, py: { xs: 1.5, md: 2.5 }, minWidth: 0 }}>
            {showShimmerState ? (
              ...
              >
                <Skeleton variant="rectangular" animation="wave" sx={{ height: 44, borderRadius: 1 }} />
                <Skeleton variant="rectangular" animation="wave" sx={{ height: 44, borderRadius: 1 }} />
                <Skeleton variant="rectangular" animation="wave" sx={{ height: 44, borderRadius: 1 }} />
                <Skeleton variant="rectangular" animation="wave" sx={{ height: 44, borderRadius: 1 }} />
                <Skeleton variant="rectangular" animation="wave" sx={{ height: 44, borderRadius: 1 }} />
